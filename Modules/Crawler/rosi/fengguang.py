...
class GetRosiUrl(HttpClient):
    def __init__(self):
        self.__pageIndex = 1
        self.__Url = "http://www.5442.com/tag/fengguang/"
        self.__refer = "http://www.5442.com/tag/fengguang.html"
...
class DownloadImg(HttpClient):
    ...
    def start(self):
        while True:
            ...
            #创建与图片集名称相同的子目录
            self.__folder = os.getcwd() + "\\fengguang\\" + title.decode("gbk", 'ignore')
...
if __name__ == '__main__':
    if not os.path.exists("fengguang"):
        os.mkdir("fengguang")
    #新建5个线程 等待队列
    for i in range(5):
        #print i
