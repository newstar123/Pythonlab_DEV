...
#   日期：2016-02-15
#   语言：Python 2.7.10
#   说明：爬取http://www.5442.com/tag/rosi/网站妹子图
...
class GetRosiUrl(HttpClient):
    ...
    def __getAllPicUrl(self, pageIndex):
        ...
        result = re.findall(pattern, page.decode("gb2312",'ignore').encode(type))
        for item in result:
            #print item
            global q
            q.put(item)
    ...
    def __getNewestPage(self):
        ...
        ul_result = re.search(pattern_ul, page.decode('gb2312'))
...
class DownloadImg(HttpClient):
    ...
    def __getMaxPage(self):
        ...
        ul_result = re.search(pattern_ul, page.decode('gb2312', 'ignore'))
        pattern_page = re.compile('[0-9]+')
    ...
    def __getAllPicUrl(self, pageIndex):
        ...
        pattern_contents = re.compile('<p align="center" id="contents">(.*?)</p>', re.S)
        imgs = re.search(pattern_contents, page.decode('gb2312', 'ignore')).group(1)
        pattern_img = re.compile('<img src=(.*?) alt=.*?/>')
    ...
    def start(self):
        while True:
            ...
            #创建与图片集名称相同的子目录
            self.__folder = os.getcwd() + "\\rosi\\" + title.decode("gbk", 'ignore')
