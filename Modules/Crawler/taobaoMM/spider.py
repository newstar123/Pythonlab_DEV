...
class Spider(object):
    ...
    def getContent(self, pageIndex):
        ...
        pattern = re.compile('<div class="pic s60".*?<a href="(.*?)".*?<img src="(.*?)".*?<a class="lady-name".*?>(.*?)</a>.*?<strong>(.*?)</strong>.*?<span>(.*?)</span>', re.S)
        items = re.findall(pattern, page)
        contents = []
        infoURL = "https://mm.taobao.com/687471686.htm?spm=719.6642053.a312r.18.no1q4w"
        for item in items:
            #网站改版，原始URL和跳转URL不一样，这里用一个手动获取的URL演示
            #contents.append(["http:" + item[0], "http:" + item[1], item[2], item[3], item[4]])
            contents.append([infoURL, "http:" + item[1], item[2], item[3], item[4]])
        return contents
    ...
    def getAllImg(self, page):
        ...
        imagesHandled = []
        for item in images:
            imagesHandled.append("http:" + item)
