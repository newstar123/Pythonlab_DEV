...
class appClass:
    ...
    def putInQueue(self, totalPage):
        num = 1
        self.log.info('#################### Delete Inactive Applications ####################')
        for onePage in range(1, totalPage+1):
            ...
            for item in application_list:
                if not item['reporting']:
                    self.q.put(item['id'])
                    self.log.info('Put id='+str(item['id'])+':name='+str(item['name'])+':reporting='+str(item['reporting'])+' in the queue, number '+str(num)+'.')
                    num = num+1
    ...
    def makeDelete(self):
        ...
        for item in range(1, self.q.qsize()+1):
            ...
            try:
                ...
                self.log.info(str(application_info['id'])+':'+str(application_info['name'])+' has been deleted.')
                # self.log.info(url + " has been deleted")
            except Exception, e:
                self.log.info('Delete '+qNumber+' failed.')

        self.log.warn('Deleting completed.')

    def deleteById(self, id):
        self.log.info('Deleting inactive application, id is ' + id)
        url = self.appBasicUrl + id + '.json'
        try:
            # data = self.myUtils.curlDelete(url)
            # server_info = data['server']
            # logging.warning(str(server_info['id'])+':'+str(server_info['name'])+' has been deleted.')
            self.log.info(url + " has been deleted")
        except Exception, e:
            self.log.info('Delete ' + id + ' failed. Error: '+str(e))
